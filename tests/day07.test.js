...
test('day 7-1 receive correct thruster signal', () => {
  const opcode = [3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0];
  const phase = [4,3,2,1,0];
  expect(day07.ampfliers(opcode, phase)).toBe(43210);
});
...
test('day 7-2 receive correct thruster signal', () => {
  ...
  expect(day07.ampfliers(opcode, phase)).toBe(139629729);
});
...
test('day 7-2 receive max thruster signal', () => {
  ...
  const signals = day07.signals_loop(opcode);
